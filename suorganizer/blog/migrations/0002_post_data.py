...
blog post information, instead of starting with an empty database.
"""
...
POSTS = [
    {
        "title": "Why I Quit My Job In 2019",
        "slug": "why-i-quit-my-job",
        "pub_date": date(2019, 10, 29),
        "startups": ["ranajay-on-the-road"],
        "tags": ["self-improvement"],
        "text": "This is the story of why I quit my job in 2019."
                "."
                "."
                "."
                "(because I was bored. The end.)",
    },
    {
...
def add_post_data(apps, schema_editor):
    ...
    for post_dict in POSTS:
        ...
        for startup_slug in post_dict['startups']:
            post.startups.add(Startup.objects.get(slug=startup_slug))


def remove_post_data(apps, schema_editor):
    Post = apps.get_model('blog', 'Post')
    for post_dict in POSTS:
        post = Post.objects.get(slug=post_dict['slug'])
        post.delete()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(add_post_data, remove_post_data)
    ]
